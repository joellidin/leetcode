    ...
    loop {
        ...
        if random_pattern.is_match(id_arg) {
            ...
            writeln!(lib_file, "{}", mod_file_addon.lock().unwrap().join("\n"))
                .expect("src/problem.rs write failed");
            break;
        } else {
            id = id_arg
                .parse::<u32>()
        ...
        let problem = fetcher::get_problem(id).unwrap_or_else(|| {
            panic!(
                "Error: failed to get problem #{id} \
...
fn get_initialized_ids() -> Vec<u32> {
    let content = fs::read_to_string("./src/problem.rs").unwrap();
...
fn deal_solving(id: &u32) {
    ...
    let lines: Vec<String> = io::BufReader::new(File::open(mod_file).unwrap())
        ...
        .collect();
    fs::write(mod_file, lines.join("\n")).expect("src/problem.rs has to exist.");
    // insert into solution.rs
    let mut lib_file = fs::OpenOptions::new()
        .append(true)
        .open("./src/solution.rs")
        .unwrap();
    writeln!(lib_file, "mod {solution_name};").expect("src/solution.rs has to exist.");
}
...
fn deal_problem(problem: &Problem, code: &CodeDefinition, write_mod_file: bool) {
    ...
    if write_mod_file {
        let mut lib_file = fs::OpenOptions::new()
            ...
            .open("./src/problem.rs")
            .unwrap();
        writeln!(lib_file, "mod {file_name};").expect("src/problem.rs has to exist.");
    }
}
